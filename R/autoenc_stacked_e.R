...
#' @description Creates a stacked autoencoder with stage-wise configurable latent sizes
#'   and dense sub-architectures through a Python/PyTorch backend.
#'
#' @details
#' The stacked autoencoder now supports progressively different latent widths across
#' stages. Keep `k = 3` and `encoding_sizes = NULL` to repeat the original bottleneck,
#' or use `encoding_sizes = c(16L, 8L, 4L)` to progressively compress the representation.
#' `encoder_hidden_sizes` and `decoder_hidden_sizes` may be either a single integer
#' vector shared by all stages or an R list with one integer vector per stage.
#'
#' @param input_size Integer. Number of input features per observation.
#' @param encoding_size Integer. Default latent size used when `encoding_sizes = NULL`.
#' @param encoding_sizes Optional integer vector. Stage-specific latent sizes. If supplied,
#'   it defines the number of stages and overrides `k`.
#' @param encoder_hidden_sizes Integer vector shared by all stages, or a list of integer
#'   vectors with one encoder architecture per stage.
#' @param decoder_hidden_sizes Optional integer vector or list mirroring
#'   `encoder_hidden_sizes`. If `NULL`, each stage mirrors its encoder in reverse order.
#' @param activation Character. Hidden activation function. One of
#'   `"relu"`, `"leaky_relu"`, `"elu"`, `"gelu"`, `"selu"`, or `"tanh"`.
#' @param output_activation Character. Output activation used by stage decoders. One of
...
#' @param k Integer. Number of stacked stages when `encoding_sizes = NULL`.
...
#' @import reticulate
#' @export
autoenc_stacked_e <- function(input_size, encoding_size,
                              encoding_sizes = NULL,
