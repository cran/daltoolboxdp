...
#' @description Creates a dense autoencoder that learns a latent representation
#'   for a sequence of observations through a Python/PyTorch backend.
#'
#' @details
#' The dense autoencoder is now architecture-configurable. You can keep the
#' original single hidden layer with `encoder_hidden_sizes = 64`, or define
#' deeper asymmetric encoder/decoder stacks such as
#' `encoder_hidden_sizes = c(128L, 64L, 32L)` and
#' `decoder_hidden_sizes = c(32L, 64L, 128L)`.
#'
...
#'   Default is `64L`, matching the previous implementation.
#' @param decoder_hidden_sizes Optional integer vector. Hidden sizes used by the
#'   decoder. If `NULL`, the decoder mirrors `encoder_hidden_sizes` in reverse order.
...
#' @import reticulate
#' @export
autoenc_e <- function(input_size, encoding_size,
